...
def main():
    ...
    expert_consensus = miami_pitt.get(
    ...
    if expert_consensus:
        ...
        for i, consensus in enumerate(expert_consensus, 1):
            ...
            print(
                f"         Odds: {consensus.get('odds_american', consensus.get('odds', 'N/A'))}")
            print(
                f"         Star Rating: {consensus.get('star_rating', 'N/A')}")
    ...
    print(
        f"   {miami_pitt_odds.get('away_team')} @ {miami_pitt_odds.get('home_team')}")
        ...
        if dk:
            ...
            for market in markets:
                ...
                for outcome in outcomes:
                    print(
                        f"         {outcome.get('name')}: {outcome.get('price')} (line: {outcome.get('point', 'N/A')})")
    ...
    if expert_consensus:
        valid_picks = [c for c in expert_consensus if c.get(
            'market') in ['spread', 'totals', 'total']]
        extreme_ml = [c for c in expert_consensus if c.get(
            'market') == 'moneyline' and abs(c.get('odds_american', 0)) > 150]
        ...
        if valid_picks:
            ...
            for pick in valid_picks:
                print(
                    f"   - {pick.get('source')}: {pick.get('market')} {pick.get('line')} @ {pick.get('odds', pick.get('odds_american'))}")
        else:
