...
def test_scrapers_and_api():
    ...
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute("DELETE FROM prompt_context WHERE sport = ? AND match_date >= ?",
                    (sport_name, target_date))
        conn.commit()
    ...
    if oddsshark_results:
        print(
            f"   Sample: {oddsshark_results[0]['game_id']} - {oddsshark_results[0]['team_pick']}")
    ...
    if cbs_results:
        print(
            f"   Sample: {cbs_results[0]['game_id']} - {cbs_results[0]['team_pick']}")
    ...
    if games:
        ...
        print(
            f"Expected 4-5 star picks: {multi_source_games} (if consensus agrees)")
